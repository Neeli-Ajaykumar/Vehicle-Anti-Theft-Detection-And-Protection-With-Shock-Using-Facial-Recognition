...
void setup() 
{
  ...
  pinMode(buzzer, OUTPUT);  pinMode(leds, OUTPUT);
...
finallong1 = String(finallong);
  ...
  lcd.clear();
  lcd.setCursor(0,0);lcd.print("Ign:");//4,0                
  lcd.setCursor(8,0);lcd.print("Veh:");//12,0
 ...
 if(digitalRead(ign) == LOW)
   {
    ...
    if(sts1 == 1)
      {
        //Serial2.print("*Ign_ON_Some_one_starting_your_vehicle#");
        Serial2.write('*');
        for(ii=0;ii<=6;ii++) Serial2.write(finallat[ii]);
        for(ii=0;ii<=7;ii++) Serial2.write(finallong[ii]);  
        Serial2.write('#');
          
        gsm_send("Ign_ON_Some_one_starting_your_vehicle");
      }
   }
